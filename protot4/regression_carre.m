...
%
% INPUT :
%   spectre_carre : vecteur du spectre d'amplitude du signal carré
%
%***********************************************************************

%% Extraction aux indices exacts
idx_harm = [4, 10, 16, 22, 28, 34, 40, 46];
v_clean  = spectre_carre(idx_harm);
%% Droite initiale fixée sur n1 et n2
...
while i <= length(v_clean)
    ...
    % Mémorisation pour affichage
    n_pred          = [n_pred, i];
    vals_pred_graph = [vals_pred_graph, val_predite];

    if ~seuil_ok
        % Phase 1 : on compare encore avec la vraie valeur
        ...
        if correspondance >= 0.90
            ...
            seuil_ok = true;
            % La droite reste celle en cours, on utilise val_predite pour la suite
        ...
        % Phase 2 : on prédit sans comparer, on enchaîne sur la valeur prédite
...
% Droite de régression finale sur une plage continue pour l'affichage
